...
from .models import TaskType, Task, TaskType1, TaskType2, TaskType2Variant

from mptt.admin import MPTTModelAdmin
from .models import TaskSection



class TaskSectionAdmin(MPTTModelAdmin):
    pass
...
def task_edit_pre(context, number):
    ...
    context['title'] = context.get('title', '') + ' ' + str(number)
...
def task_edit_details(request, number):
    ...
    if task_option.type.id == 1:
        ...
        task.explanation = request.POST.get('explanation')
        if request.FILES.get('explanation_image'):
            task.explanation_image = request.FILES.get('explanation_image')
            task.explanation_image.name = 'task-' + str(task_option.number) + '-explanation-image.jpg'
...
def task_edit(request):
    ...
    if request.method == 'GET':
        ...
        task_option = Task.objects.get(number=number)
        context['title'] = 'Task:' + str(task_option.number) + ' Seccess!'
        context['task_option'] = task_option

    return render(request, template, context)


@admin.site.register_view('add-task/', 'Add new task')
def task_add(request):
    ...
    if request.method == 'POST':
        ...
        context['title'] = 'Choose task type: '
